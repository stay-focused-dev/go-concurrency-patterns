...
func realFetch(domain string) Fetcher {
	fp := rss.NewParser()
	uri := fmt.Sprintf("http://%s/feeds/posts/default?alt=rss", domain)
	return &realFetcher{parser: fp, uri: uri}
}

func (f *realFetcher) Fetch() (items []Item, next time.Time, err error) {
	var feed *rss.Feed
	if feed, err = f.parser.ParseURL(f.uri); err != nil {
		return
	}

	for _, item := range feed.Items {
		items = append(items, Item{Title: item.Title, Channel: feed.Title, GUID: item.GUID})
	}
	next = time.Now().Add(12 * time.Minute)
	return
}
